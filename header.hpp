...
struct Broker {
    std::string name;
    std::vector<std::pair<unsigned int, std::pair<unsigned int, unsigned int>>> account_inf;  // номер счета-количество файлов-дата последнего
};
        ...
        if (boost::filesystem::is_directory(x.path())) {
            ...
            for (auto &a : brokerVector) {
                if (broker.name == a.name) {
                    ...
                    if (!existAccount) {
                        a.account_inf.push_back(std::make_pair(bill, std::make_pair(1, date)));
                    }
                    existAccount = false;
                }
            }
            if (!existBroker) {
                broker.account_inf.push_back(std::make_pair(bill, std::make_pair(1, date)));
...
void print() {
    for (const auto &x : brokerVector) {
        for (const auto &y : x.account_inf) {
            std::cout << "broker:" << x.name << " account:" << y.first << " files:" << y.second.first << " lastdate:" << y.second.second << std::endl;
        }
    }
}
